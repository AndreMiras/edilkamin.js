...
import {transformMacAddress} from './utils';
...
const deviceInfo =
    (axiosInstance: AxiosInstance) => (jwtToken: string, macAddress: string) =>
        axiosInstance.get<DeviceInfoType>(`device/${transformMacAddress(macAddress)}/info`, {
...
const mqttCommand =
    (axiosInstance: AxiosInstance) =>
        (jwtToken: string, macAddress: string, payload: any) =>
            axiosInstance.put(
                'mqtt/command',
                {mac_address: transformMacAddress(macAddress), ...payload},
                {headers: headers(jwtToken)}
            );
